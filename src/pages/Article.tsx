...
import { CheckCircle2, ChevronRight, HelpCircle, Tag, Clock, CalendarDays } from "lucide-react";
...
export default function Article() {
  ...
  return (
    <div className="flex flex-col w-full py-8 md:py-12 bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        <header className="mb-12 border-b border-slate-200 pb-10">
          ...
          <div className="flex items-center text-sm text-slate-500 font-medium">
            ...
            {Math.max(3, Math.ceil(article.steps.length * 1.5))} min read
          </div>
        </header>
        ...
        <article className="prose prose-lg prose-blue max-w-none">
          <p className="lead text-lg mb-8">
            If you are experiencing this issue, don't panic. Follow these proven troubleshooting steps sequentially. We start with the easiest, least invasive fixes and progress to more advanced solutions.
          </p>
          ...
          <div className="bg-green-50 border border-green-200 rounded-2xl p-6 my-10 flex items-start">
            ...
            <div>
              <h4 className="font-bold text-green-900 m-0 text-lg">Did this resolve your issue?</h4>
              <p className="text-green-800 m-0 mt-1">If the steps above fixed your problem, make sure to keep your system updated to prevent it from happening again.</p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              {article.faqs.map((faq, index) => (
                <div key={index} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <p className="text-slate-600 m-0 ml-7 leading-relaxed">
                    {faq.answer}
                  </p>
                </div>
              ))}
      ...
      {/* Related Articles (Full Width Background) */}
      <div className="w-full bg-slate-50 py-16 border-t border-slate-200 mt-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-slate-900 mb-8 font-display">Related Fixes</h2>
