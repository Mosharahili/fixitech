...
import { getStepsForLang, getFAQsForLang } from "@/data/step-templates";
...
export default function Article() {
  ...
  const relatedArticles = allArticles.filter(a => article.relatedSlugs.includes(a.slug));

  const trans = lang !== "en" ? article.translations?.[lang as "ar" | "es"] : undefined;
  const displayTitle = trans?.title || article.title;
  const displayDescription = trans?.description || article.description;
  const displayIntro = trans?.intro || article.intro;
  const displayKeywords = trans?.keywords || article.keywords;

  const displaySteps = article.type && lang !== "en"
    ? getStepsForLang(article.type, lang)
    : article.steps;

  const templateFAQs = article.type && lang !== "en" ? getFAQsForLang(lang) : [];
  const displayFAQs = templateFAQs.length > 0 ? templateFAQs : article.faqs;
        ...
        <header className="mb-12 border-b border-slate-200 pb-10">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 font-medium">
            ...
            <span className="flex items-center">
              ...
              {Math.max(3, Math.ceil(displaySteps.length * 1.5))} {t("minRead")}
            </span>
          </div>
        </header>
